...
| Check JSON payload shape              | Top-level route list is canonical and `run_result` stays lightweight | PASS    | Report validation confirms JSON keeps one full `route_decisions` list at top level and does not duplicate it inside nested `run_result`. |
...
| Check warning count consistency                     | `summary.warnings`, visible warnings, and `warning_events` stay aligned             | PASS    | Focused warning validation confirms ignore-string warning count stays consistent across summary, warnings, and `warning_events`. |
